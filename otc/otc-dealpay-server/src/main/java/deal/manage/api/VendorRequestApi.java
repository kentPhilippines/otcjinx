...
public class VendorRequestApi {
    ...
    public Result pay(HttpServletRequest request) {
        ...
        if(StringUtils.isNotEmpty(userInfo.getMinAmount().toString()) && StrUtil.isNotBlank(userInfo.getMaxAmount().toString())){
            if(orderAmount <= Double.parseDouble(userInfo.getMinAmount().toString()) || orderAmount >= Double.parseDouble(userInfo.getMaxAmount().toString())){
