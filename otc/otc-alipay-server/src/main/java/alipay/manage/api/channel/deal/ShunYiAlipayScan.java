...
public class ShunYiAlipayScan extends PayOrderService{
	...
	public Result deal(DealOrderApp dealOrderApp, String payType) {
		...
		if(StrUtil.isNotBlank(create)) {
			...
			PddBean createOrder = createOrder(dealOrderApp,config.getResult()+PayApiConstant.Notfiy.NOTFIY_API_WAI+"/shunyi-notfiy", dealOrderApp.getOrderAmount(),create);
	...
	PddBean createOrder(DealOrderApp dealOrderApp,String notfiy,BigDecimal bigDecimal, String orderId){
		/**
	...
	if(ObjectUtil.isNotNull(bean)) {
		...
		//	{"ret_code":"4008","ret_msg":"服务未开通"}
			orderEr(dealOrderApp,"顺易返回："+bean.getRet_msg());
		}
	}
	return null;
	}
